...
Here, ``unimolecularThreshold``, ``bimolecularThreshold``, and ``trimolecularThreshold`` are binary arrays storing
...
where :math:`k_{threshold} = filterThreshold`. ``filterThreshold`` is adjusted by evaluating Arrhenius fits for each
...
where :math:`k_{threshold} = 10^{-3} \cdot filterThreshold \frac{m^6}{mol^2\cdot s}`. Based on extending Smoluchowski
